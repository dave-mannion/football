...
def format_match_lineup_json(match_lineup_json):
    ...
    home_starters = {f'home_starter_id_fl_{i+1}':x for i,x in enumerate(starter_ids[home_team])}
    away_starters = {f'away_starter_id_fl_{i+1}':x for i,x in enumerate(starter_ids[away_team])}
